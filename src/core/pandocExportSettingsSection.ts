...
} from '../pandoc/odtPreviewAddon';
import { joinPath } from '../pandoc/pathUtils';
...
export function renderPandocExportSettingsSection(
    ...
    renderPreviewSettings(plugin, containerEl);
...
function renderPreviewSettings(
    ...
    renderOdtAddonSettings(plugin, containerEl);
}
...
function renderOdtAddonSettings(
    ...
    new Setting(containerEl)
        ...
        .addButton(button => button
            ...
            .onClick(async () => {
                if (addon.status !== 'installed') return;
                settings.preview.odtAddon = await removeOdtPreviewAddon(addon);
                await plugin.saveSettings();
                // eslint-disable-next-line obsidianmd/ui/sentence-case
