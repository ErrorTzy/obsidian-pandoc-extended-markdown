...
This guide records the design philosophy, supported syntax, current
implementation, and edge cases that future fenced-div changes should keep
aligned across Live Preview, Reading mode, and Pandoc export.
...
3. Numbering is not tied to visual nesting. Use "manual depth" or "counter
   depth", and do not imply automatic nested numbering.
4. Any unescaped `&` starts numbering. Literal ampersands such as `AT&T` must be
...
5. Only the first placeholder group is replaced. Additional unescaped `&`
...
9. The Lua filter is aligned with the core behavior through tests, but it is a
