...
import type { PandocOptionsMetadata } from '../../../src/pandoc/core';
...
`)
    ...
    it('uses the fallback catalog when no process service is injected', async () => {
        await expect(new PandocCatalogService().loadCatalog()).resolves.toEqual(FALLBACK_PANDOC_CATALOG);
    });
...
`);
    ...
    it('quotes Windows path tokens without doubling path separators', () => {
        expect(quoteTokenForPlatform(
            'C:\\Users\\Scott\\My Note.md',
            'windows'
        )).toBe('"C:\\Users\\Scott\\My Note.md"');
        expect(quoteTokenForPlatform(
            'C:\\Images;D:\\Shared',
            'windows'
        )).toBe('"C:\\Images;D:\\Shared"');
    });
});
